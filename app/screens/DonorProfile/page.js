...
export default function DonorProfile() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async u => {
      if (u) { setUser(u); const snap = await getDoc(doc(db, "profiles", u.uid)); if (snap.exists()) { const d = snap.data(); setProfile(d); setName(d.name || ""); setPhone(d.phone || ""); } }
  ...
  const save = async () => {
    setSaving(true);
    await setDoc(doc(db, "profiles", user.uid), { name, phone, role: "donor" }, { merge: true });
  ...
  return (
    <main style={{ maxWidth: 390, margin: "0 auto", minHeight: "100vh", background: "#F5F2ED", fontFamily: "sans-serif", paddingBottom: 80 }}>
      <div style={{ background: "#1C3123", padding: "48px 24px 32px", borderRadius: "0 0 28px 28px", textAlign: "center" }}>
        ...
        <span style={{ background: "#F5F2ED20", color: "#F5F2ED", fontSize: 12, padding: "4px 12px", borderRadius: 999 }}>💛 Donor</span>
      </div>
      <div style={{ padding: "20px 20px 0" }}>
        {!editing ? (
          <>
            <div style={{ background: "#fff", borderRadius: 18, padding: 18, border: "1px solid #1C312210", marginBottom: 12 }}>
              {[["Name", profile?.name || "—"], ["Email", user?.email || "—"], ["Phone", profile?.phone || "—"], ["Total donated", "Rs 7,500"]].map(([label, val], i, arr) => (
