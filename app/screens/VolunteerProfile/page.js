...
export default function VolunteerProfile() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async u => {
      if (u) { setUser(u); const snap = await getDoc(doc(db, "profiles", u.uid)); if (snap.exists()) { const d = snap.data(); setProfile(d); setName(d.name || ""); setPhone(d.phone || ""); setSkills(d.skills || []); } }
  ...
  const save = async () => {
    setSaving(true);
    await setDoc(doc(db, "profiles", user.uid), { name, phone, skills, role: "volunteer" }, { merge: true });
    setProfile(p => ({ ...p, name, phone, skills }));
  ...
  return (
    <main style={{ maxWidth: 390, margin: "0 auto", minHeight: "100vh", background: "#F5F2ED", fontFamily: "sans-serif", paddingBottom: 80 }}>
      <div style={{ background: "#1C3123", padding: "48px 24px 32px", borderRadius: "0 0 28px 28px", textAlign: "center" }}>
        <div style={{ width: 72, height: 72, borderRadius: "50%", background: "#F5F2ED", color: "#1C3123", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 28, margin: "0 auto 12px" }}>{name?.charAt(0)?.toUpperCase() || "?"}</div>
        ...
        <span style={{ background: "#F5F2ED20", color: "#F5F2ED", fontSize: 12, padding: "4px 12px", borderRadius: 999 }}>🤝 Volunteer</span>
      </div>
        ...
        {!editing ? (
          <>
            <div style={{ background: "#fff", borderRadius: 18, padding: 18, border: "1px solid #1C312210", marginBottom: 12 }}>
              {[["Name", profile?.name || "—"], ["Phone", profile?.phone || "—"], ["Skills", profile?.skills?.join(", ") || "—"]].map(([label, val], i, arr) => (
                <div key={label} style={{ display: "flex", justifyContent: "space-between", paddingBottom: i < arr.length - 1 ? 12 : 0, marginBottom: i < arr.length - 1 ? 12 : 0, borderBottom: i < arr.length - 1 ? "1px solid #f0f0f0" : "none" }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 500, color: "#1C3123", margin: 0, maxWidth: "60%", textAlign: "right" }}>{val}</p>
                </div>
              ))}
            </div>
            ...
            <button onClick={() => router.push("/screens/Partnership")} style={{ width: "100%", padding: 14, background: "#FAEEDA", color: "#633806", border: "1px solid #FAC77560", borderRadius: 16, fontSize: 15, fontWeight: 700, cursor: "pointer", marginBottom: 10 }}>
  ⭐ Upgrade to Premium
</button>
          ...
          <>
            <div style={{ background: "#fff", borderRadius: 20, padding: 20, border: "1px solid #1C312210", marginBottom: 16 }}>
              {[["Full name", "text", "Your name", name, setName], ["Phone", "tel", "+92 300 ...", phone, setPhone]].map(([label, type, ph, val, set]) => (
                <div key={label}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {allSkills.map(s => (
                  <button key={s} onClick={() => toggleSkill(s)} style={{ padding: "7px 14px", borderRadius: 999, border: "none", fontSize: 12, fontWeight: 500, cursor: "pointer", background: skills.includes(s) ? "#1C3123" : "#F5F2ED", color: skills.includes(s) ? "#F5F2ED" : "#1C312270" }}>{s}</button>
                ))}
              </div>
            </div>
